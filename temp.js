...
(function() {
  ...
  var settingData = {
    ...
    rules: {
      rule_def: {
        name: "default",
        hook_eventNames:
          "contextmenu|select|selectstart|copy|cut|dragstart|mousemove|beforeunload",
      ...
      rule_plus: {
        name: "default",
        hook_eventNames:
          "contextmenu|select|selectstart|copy|cut|dragstart|mousedown|mouseup|mousemove|beforeunload",
      ...
      rule_zhihu: {
        name: "default",
        hook_eventNames:
          "contextmenu|select|selectstart|copy|cut|dragstart|mousemove",
        unhook_eventNames: "keydown|keyup",
        dom0: true,
        hook_addEventListener: true,
        hook_preventDefault: true,
  ...
  if (check_black_list(list, hostname)) {
    try {
      ...
      console.error(
        "脚本rwl-错误：\n btn_node : %s\n%s\n脚本rwl-错误位置： btn_node.checked = true;",
        btn_node,
        e
      );
    } finally {
  ...
  function addBtn() {
    ...
    node.style.cssText =
      ...
      tempHeight +
      "px;left:" +
      rwl_userData.positionLeft +
      "px;right:" +
      rwl_userData.positionRight +
      "px;";
    ...
    node.innerHTML =
      '<button type="button" id="rwl-setbtn"> set </button> <lalala style="cursor:move;">限制解除</lalala> <input type="checkbox" name="" id="black_node" >';
  ...
  document.querySelector("#rwl-setbtn").addEventListener("click", function() {
    ...
    var innerH =
      "" +
      "<p>距离顶部距离（单位 像素） <input id='positiontop' type='text' value=" +
      userSetting.positionTop +
      "></p>" +
      "" +
      "<laberl> <p>允许上传黑名单<input id='uploadchecked'  type='checkbox' " +
      upload_checked +
      "></p>" +
      "</laberl>" +
      "<p><s>显示按钮（待添加）</s></p>" +
      ...
      JSON.stringify(userSetting.data, false, 4) +
      "</textarea>" +
      "<br>" +
  ...
  function saveSetting() {
    ...
    var uploadChecked = document.querySelector("#rwl-setMenu #uploadchecked")
      .checked;
  ...
  function init() {
    ...
    if (rule.hook_set_returnValue) {
      Event.prototype.__defineSetter__("returnValue", function() {
        if (
          ...
          hook_eventNames.indexOf(this.type) >= 0
        ) {
          this.returnValue = true;
        }
      });
    ...
    if (rule.add_css) {
      GM_addStyle(
        'html, :not([class*="rwl-exempt"]) {-webkit-user-select:text!important; -moz-user-select:text!important;} :not([class*="rwl-exempt"]) ::selection {color:#fff; background:#3390FF; !important;}'
      );
    } //else {
